...
Meteor.methods({ //Ajustamos los eventos de meteor
  ...
  upvote: function(postId) { //método upvote recibirá como parámetro el postId
    ...
    var affected = Posts.update({ //la variable affected guardara el callback de lo que genere update si
      _id: postId, //existe un post con este id
      upvoters: {$ne: this.userId} //el post con el id en upvoters no encuentra a this.userId
    }, { //y de pasar lo primero
      $addToSet: { //añadirá
        upvoters: this.userId //en upvoters el userId del usuario que votó
      },
      $inc: { //e incrementará
        votes: 1 //en uno el valor de la propiedad votes
      }
    });

    if (! affected) { //De no haberse producido el voto (affected == false)
      throw new Meteor.Error('invalid', "You weren't able to upvote that post"); //Error
    }

    //Guardo este código como referencia del funcionamiento
